...
function initializeVisia() {
	...
	jQuery('.form-click').click(function(){
    ...
    var ipinfo = jQuery('#message').val();

    var dbinfo = "";

    jQuery('input:checked').each(function(i,item){
      dbinfo += "\n試用資料庫: " + item.value;
    })

    if (!name || !phone || !address || !contact || !ipinfo || !dbinfo){
      alert("請填寫所有欄位，Please fill up all fields, Thanks.")
      return false;
    }
    ...
    // post to send mail
		jQuery.post(action, {
			...
			function(data){
				//document.getElementById('alert').innerHTML = "Thanks";
        alert("Thank");
				jQuery('#alert').html("Thanks").show();
